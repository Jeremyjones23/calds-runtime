...
<decomposition>
  ...
  <failure_modes>
    ...
    <failure_mode>The reader sees source completeness 0 / 100 or publication confidence 30 / 100 and cannot tell whether that means the investigation failed, the report is broken, or the case is only internally reviewable.</failure_mode>
...
<optimized_prompt>
  ...
  <execution_rules>
    ...
    <rule>Explain low source completeness using missing-data count and contradiction count.</rule>
    <rule>Explain low publication confidence using source completeness and source diversity.</rule>
...
<stress_test>
  <phase name="Literal Misread">
    ...
    <misread>The reader interprets source completeness 0 / 100 as meaning CalDS has no evidence or that the report failed.</misread>
    <failure>The reviewer underweights a high-risk internal lead or mistrusts the score.</failure>
    <exploited_ambiguity>Completeness sounds like document quality or evidence existence.</exploited_ambiguity>
    <neutralizer>The prompt requires score scope and formula explanation, including missing-data and contradiction counts.</neutralizer>
  </phase>
...
<compression_report>
  ...
  <constraints_added>Future-case structure lock, source-fact-first opening, compact narrative citation labels, selected-vs-watchlist separation, no data-gap-as-adverse-fact rule, score-scope explanation, low-score cause explanation, cold-reader question prompts, and public reader acronym expansion.</constraints_added>
  <failure_modes_covered>Noisy first page, missing case thesis, uncited opinions, confused entity selection, acronym confusion, source-gap overstatement, unsupported allegations, source-identifier corruption, score misinterpretation, and one-off case customization.</failure_modes_covered>
