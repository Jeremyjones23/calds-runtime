...
<optimized_prompt>
  ...
  <constraints>Use only supplied artifacts. Do not add facts, infer wrongdoing, or rewrite the risk posture. Do not publish local paths, raw downloaded source caches, secrets, or private archive paths. Preserve evidence IDs, record IDs, checksums, sentinel decision, caveats, and human-review pause. Do not claim an internet citation exists unless an actual URL is present. Every public source-ledger row must contain source URLs or an explicit not-linkable note.</constraints>
