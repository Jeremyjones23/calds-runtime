...
def validate_case_output(case: CaseRequest, run_dir: Path) -> dict[str, object]:
    ...
    assert_true("missing-data signal" in dossier_text, "case dossier does not explain source-completeness penalty")
    assert_true("source completeness at 70%" in dossier_text, "case dossier does not explain publication-confidence formula")
    ...
    assert_true("Who they are / what they say they do" in dossier_text, "case dossier missing source-backed organization context")
    assert_true("What CalDS found in the records" in dossier_text, "case dossier missing briefing facts")
    ...
    assert_true("Reviewer readout" in dossier_text, "case dossier missing briefing rationale")
    assert_true("Recommended human next step" in dossier_text, "case dossier missing supervisor next step")
